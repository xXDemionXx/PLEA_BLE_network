...
def notification_loop(peripheral, stop_event):
    ...
    while not stop_event.is_set():
        try:
            ...
            currently_connected_networks = subprocess.run(['nmcli', '-t', '-f', 'NAME', 'connection', 'show', '--active'], capture_output=True, text=True).stdout
            if previous_connected_networks != currently_connected_networks:		# Check if any network connection has changed
...
def BLE_send_networks_connection_status_message(connected_networks):
	...
	BLE_connected_networks_message_array = BLE_chop_string_to_chunks(BLE_connected_networks_message, 20)
	#print(BLE_connected_networks_message_array)
	BLE_send_array(BLE_connected_networks_message_array, network_message_ch)	# Send the connection message
###
